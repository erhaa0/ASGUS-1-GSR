...
import { fetchUsers, registerUser, updateUser,
         ...
         triggerDetection, updateRiskParams, fetchRiskParams, fetchHealth, fetchZones } from '../api/api';
...
export default function AdminPanel() {
    ...
    const [selectedDetectZone, setSelectedDetectZone] = useState('');
    const [detectLoading, setDetectLoading]           = useState(false);
    const [riskZoneId, setRiskZoneId]                 = useState('');
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const [usersData, logsData, healthData, zonesData] = await Promise.all([
                    fetchUsers(),
                    fetchLogs(15),
                    fetchHealth(),
                    fetchZones(),
                ...
                if (zonesData && zonesData.length > 0) {
                    setZones(zonesData);
                    setSelectedDetectZone(zonesData[0].zone_id);
                    setRiskZoneId(zonesData[0].zone_id);
                }
                if (healthData?.services) {
                    const ICONS = { 'API Server': Server, 'Supabase': Database, 'AI Microservice': Cpu,  'Supabase REST API': Cloud, 'PostGIS Extension': Layers };
    ...
    const openEditModal = (u) => {
        ...
        setFormZones(u.assigned_zone ? [u.assigned_zone] : []);
        setFormPass('');
        setModalMode('edit');
    };

    const handleZoneToggle = (zoneId) => {
        setFormZones(prev => prev.includes(zoneId) ? [] : [zoneId]);
    };
    ...
    const saveUser = async () => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                ...
                const updates = { full_name: formName };
                if (formZones[0]) updates.assigned_zone = formZones[0];
    ...
    // No hardcoded fallback — show loading state if real data not yet fetched
    const healthWidgets = [];
            ...
            <div className="main-wrapper">
                ...
                <div className="admin-page">
                    ...
                    <div className="admin-card health-strip">
                        {liveHealthData.length === 0 && (
                            <div style={{ color: '#555', fontSize: 13, padding: '12px 0' }}>Loading service health…</div>
                        )}
                        {liveHealthData.map((w, i) => (
                            ...
                            {formRole === 'Field Officer' && (
                                <div className="admin-input-group">
                                    ...
                                    <div className="admin-zones-grid">
                                        {zones.map(z => (
                                            <label key={z.zone_id} className="admin-checkbox-lbl">
                                                <input type="checkbox" checked={formZones.includes(z.zone_id)} onChange={() => handleZoneToggle(z.zone_id)} />
                                                {z.zone_name}
                                            </label>
                                        ))}
                                    </div>
