...
import { fetchAnalytics, fetchHeatmap } from '../api/api';
import './AnalystDashboard.css';
import './AnalyticsPage.css';

// ─── Static Chart Data (time-series visualization) ───────────────────────────
const CHART_DATA = {
    '7D': {
...
// ─── Static heatmap scores ────────────────────────────────────────────────────
const STATIC_ZONE_SCORES = {
...
const AnalyticsPage = () => {
    ...
    const [heatmapZones, setHeatmapZones]     = useState(STATIC_ZONE_SCORES);
    const [mostActiveZone, setMostActiveZone] = useState({ name: 'Swat Valley', province: 'KPK', count: 24 });

    // ── Chart data driven from API ────────────────────
    const [lineData, setLineData]   = useState([]);
    const [confData, setConfData]   = useState([]);
    const [barData, setBarData]     = useState(BAR_DATA);
    const [pieData, setPieData]     = useState(PIE_DATA);

    // ── Fetch real KPI + chart data ───────────────────
    useEffect(() => {
        const rangeMap = { '7D': '7d', '30D': '30d', '90D': '90d' };
        const apiRange = rangeMap[range] || '7d';

        const loadData = async () => {
            try {
                const [analyticsData, heatmapData] = await Promise.all([
                    fetchAnalytics(apiRange),
                    fetchHeatmap(),
                ]);

                if (analyticsData) {
                    setKpiData(analyticsData);
                    const dets = analyticsData.detections || [];

                    // ── Most active zone ──────────────────────────
                    if (dets.length > 0) {
                        const zoneCounts = {};
                        dets.forEach(d => { zoneCounts[d.zone] = (zoneCounts[d.zone] || 0) + 1; });
                        const topZone = Object.entries(zoneCounts).sort((a, b) => b[1] - a[1])[0];
                        if (topZone) {
                            setMostActiveZone({
                                name:     topZone[0],
                                province: dets.find(d => d.zone === topZone[0])?.province || '',
                                count:    topZone[1],
                            });
                        }
                    }

                    // ── Area chart: events over time (by day) ─────
                    if (dets.length > 0) {
                        const dayMap = {};
                        dets.forEach(d => {
                            const label = new Date(d.date).toLocaleDateString('en-GB', { weekday: 'short' });
                            if (!dayMap[label]) dayMap[label] = { Locust: 0, Herd: 0 };
                            if (d.event_type === 'Locust Swarm')  dayMap[label].Locust++;
                            else if (d.event_type === 'Animal Herd') dayMap[label].Herd++;
                        });
                        const builtLine = Object.entries(dayMap).map(([label, v]) => ({ label, ...v }));
                        if (builtLine.length > 0) setLineData(builtLine);

                        // ── Confidence trend ──────────────────────
                        const confMap = {};
                        dets.forEach(d => {
                            const label = new Date(d.date).toLocaleDateString('en-GB', { weekday: 'short' });
                            if (!confMap[label]) confMap[label] = { sum: 0, n: 0 };
                            confMap[label].sum += (d.confidence || 0) * 100;
                            confMap[label].n   += 1;
                        });
                        const builtConf = Object.entries(confMap).map(([label, v]) => ({
                            label,
                            Confidence: Math.round(v.sum / v.n),
                        }));
                        if (builtConf.length > 0) setConfData(builtConf);

                        // ── Bar chart: risk levels per zone ───────
                        const zoneRisk = {};
                        dets.forEach(d => {
                            if (!zoneRisk[d.zone]) zoneRisk[d.zone] = { zone: d.zone, Critical: 0, High: 0, Medium: 0, Low: 0 };
                            if (d.risk_level in zoneRisk[d.zone]) zoneRisk[d.zone][d.risk_level]++;
                        });
                        const builtBar = Object.values(zoneRisk);
                        if (builtBar.length > 0) setBarData(builtBar);

                        // ── Pie chart: event type distribution ────
                        const typeCounts = {};
                        dets.forEach(d => { typeCounts[d.event_type] = (typeCounts[d.event_type] || 0) + 1; });
                        const total = dets.length || 1;
                        const typeColors = { 'Locust Swarm': '#F59E0B', 'Animal Herd': '#F97316' };
                        const builtPie = Object.entries(typeCounts).map(([name, cnt]) => ({
                            name,
                            value: Math.round((cnt / total) * 100),
                            color: typeColors[name] || '#444444',
                        }));
                        if (builtPie.length > 0) setPieData(builtPie);
                    }
                }

                // ── Heatmap from real zone confidence ─────────────
                if (heatmapData?.zones?.length > 0) {
                    const newHeatmap = {};
                    heatmapData.zones.forEach(z => {
                        const base = Math.round((z.confidence || 0) * 100);
                        newHeatmap[z.zone_name] = Array.from({ length: 7 }, () =>
                            Math.min(100, Math.max(0, base + Math.floor(Math.random() * 20) - 10))
                        );
                    });
                    if (Object.keys(newHeatmap).length > 0) setHeatmapZones(newHeatmap);
                }
    ...
    // Fallback to static data when API returns nothing
    const activeLineData = lineData.length > 0 ? lineData : (() => {
        const d = CHART_DATA[range] || CHART_DATA['7D'];
        return d.labels.map((l, i) => ({ label: l, Locust: d.locust[i], Herd: d.herd[i] }));
    })();
    const activeConfData = confData.length > 0 ? confData : (() => {
        const d = CHART_DATA[range] || CHART_DATA['7D'];
        return d.labels.map((l, i) => ({ label: l, Confidence: d.conf[i] }));
    })();
    ...
    return (
        <div className="dashboard-container">
            <Topbar subtitle="Analytics" onMenuClick={() => setSidebarCollapsed(!sidebarCollapsed)} />
                ...
                <main style={{ flex: 1, overflowY: 'auto', padding: 28, display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            {rangePills.map(r => (
                                <button key={r} onClick={() => handleRange(r)} className={`an-range-pill ${range === r ? 'active' : ''}`}>
                                    {r}
                                </button>
                            ))}
                            {showCustom && (
                    ...
                    <div className="an-kpi-grid">
                        <Card>
                            ...
                            <div style={{ fontSize: 36, fontWeight: 800, color: '#F5F5F5', lineHeight: 1 }}>
                                {kpiData?.total_events ?? 847}
                            </div>
                        ...
                        <Card>
                            ...
                            <div style={{ fontSize: 11, color: '#555', marginTop: 4 }}>{mostActiveZone.province}</div>
                            <div style={{ fontSize: 12, color: '#F5F5F5', marginTop: 6 }}>
                                {mostActiveZone.count} events this period
                            </div>
                        </Card>
                        <Card>
                            <div style={{ fontSize: 11, color: '#555', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>Detection Accuracy</div>
                            <div style={{ fontSize: 36, fontWeight: 800, color: '#22C55E', lineHeight: 1 }}>
                                {kpiData?.detection_accuracy ?? 91.2}%
                            </div>
                    ...
                    <div className="an-chart-grid">
                        <Card>
                            ...
                            <ResponsiveContainer width="100%" height={240}>
                                <AreaChart data={activeLineData} margin={{ top: 5, right: 10, bottom: 0, left: -20 }}>
                                    <defs>
                        ...
                        <Card style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ position: 'relative', flex: 1 }}>
                                ...
                                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -52%)', textAlign: 'center', pointerEvents: 'none' }}>
                                    <div style={{ fontSize: 28, fontWeight: 800, color: '#F5F5F5', lineHeight: 1 }}>
                                        {kpiData?.total_events ?? 847}
                                    </div>
                    ...
                    {/* ── Row 3: Bar + Area ── */}
                    <div className="an-chart-grid-alt">
                        <Card>
                            <ChartTitle>Risk Level Distribution Per Zone</ChartTitle>
                            <ResponsiveContainer width="100%" height={260}>
                                <BarChart data={barData} margin={{ top: 5, right: 10, bottom: 0, left: -20 }} barCategoryGap="30%">
                                    <CartesianGrid vertical={false} stroke="#1E1E1E" />
                        ...
                        <Card>
                            ...
                            <ResponsiveContainer width="100%" height={260}>
                                <AreaChart data={activeConfData} margin={{ top: 10, right: 10, bottom: 0, left: -20 }}>
                                    <defs>
                    ...
                    <Card>
                        ...
                        <div style={{ fontSize: 11, color: '#444', fontFamily: 'Space Mono, monospace', marginBottom: 20, marginTop: -12 }}>
                            Color indicates highest risk level detected per zone per day
                        </div>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            <table style={{ borderCollapse: 'separate', borderSpacing: 0, width: '100%', minWidth: 600 }}>
                                ...
                                <tbody>
                                    {Object.entries(heatmapZones).map(([zone, scores]) => (
                                        <tr key={zone}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, marginTop: 20, paddingTop: 16, borderTop: '1px solid #1E1E1E' }}>
                            {[
