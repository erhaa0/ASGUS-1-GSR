...
const ZONES_DATA = [
    { name: 'Quetta', province: 'Balochistan', district: 'Quetta District', risk: 'Critical', conf: 94, type: 'Locust Swarm', time: '14:22 UTC', vel: '12 km/h NE', status: 'Active' },
    { name: 'Kech', province: 'Balochistan', district: 'Kech District', risk: 'Critical', conf: 97, type: 'Locust Swarm', time: '13:45 UTC', vel: '18 km/h NE', status: 'Active' },
    { name: 'Zhob', province: 'Balochistan', district: 'Zhob District', risk: 'High', conf: 81, type: 'Animal Herd', time: '13:10 UTC', vel: '8 km/h E', status: 'Monitoring' },
    { name: 'Pishin', province: 'Balochistan', district: 'Pishin District', risk: 'Medium', conf: 62, type: 'Animal Herd', time: '12:55 UTC', vel: '5 km/h SE', status: 'Monitoring' },
    { name: 'Swat', province: 'KPK', district: 'Swat District', risk: 'Critical', conf: 98, type: 'Locust Swarm', time: '14:15 UTC', vel: '22 km/h NE', status: 'Active' },
    { name: 'Dir', province: 'KPK', district: 'Dir District', risk: 'Low', conf: 38, type: 'Unknown', time: '11:20 UTC', vel: '3 km/h S', status: 'Cleared' },
];
...
// ─── Generate Modal ───────────────────────────────────────────────────────────
const GenerateModal = ({ zoneName, onClose, onGenerated }) => {
    ...
    const handleGenerate = () => {
        ...
        setTimeout(() => {
            ...
            a.href = url;
            a.download = `asgus1-report-${zoneName}.txt`;
            ...
            <div className="modal-card" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    <div className="input-group">
                        ...
                        <input type="text" value={zoneName} readOnly className="modal-input read-only" />
                    </div>
...
const ZoneExplorerPage = () => {
    ...
    const filteredZones = useMemo(() => ZONES_DATA.filter(z => {
        ...
        if (risk !== 'All Levels' && z.risk !== risk) return false;
        if (eventType !== 'All Types' && z.type !== eventType) return false;
        if (status !== 'All Status' && z.status !== status) return false;
        return true;
    }), [search, province, risk, eventType, status]);
    ...
    return (
        <div className="dashboard-container">
            ...
            <div className="main-wrapper">
                ...
                <main style={{ flex: 1, overflowY: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, background: '#0D0D0D', border: '1px solid #1E1E1E', borderRadius: 8, padding: '10px 20px', fontSize: 12, color: '#666', fontFamily: 'Space Grotesk, sans-serif' }}>
                        <div><span style={{ color: '#F5F5F5', fontWeight: 600 }}>{ZONES_DATA.length}</span> Total Zones</div>
                        <div style={{ width: 1, height: 16, background: '#1E1E1E' }}></div>
                        <div><span style={{ color: '#EF4444', fontWeight: 600 }}>3</span> Critical</div>
                        <div style={{ width: 1, height: 16, background: '#1E1E1E' }}></div>
                        <div><span style={{ color: '#F97316', fontWeight: 600 }}>7</span> High</div>
                        <div style={{ width: 1, height: 16, background: '#1E1E1E' }}></div>
                        <div><span style={{ color: '#22C55E', fontWeight: 600 }}>14</span> Monitoring or Cleared</div>
                    </div>
                    ...
                    {filteredZones.length === 0 ? (
                        ...
                        <div className="ze-grid">
                            {filteredZones.map(z => {
                                const rc = RISK_COLORS[z.risk];
                                return (
                                    <div key={z.name} className="ze-card" style={{ borderLeftColor: rc.solid }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                                            <div style={{ fontSize: 18, fontWeight: 800, color: '#F5F5F5' }}>{z.name}</div>
                                            <RiskPill risk={z.risk} />
                                        </div>
                                        <div style={{ fontSize: 12, color: '#555', marginBottom: 12 }}>{z.province} &middot; {z.district}</div>
                                        <div className="topbar-divider" style={{ width: '100%', margin: '12px 0' }}></div>

                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 6 }}>
                                            <span style={{ color: '#666' }}>Confidence</span>
                                            <span className="mono" style={{ color: '#F59E0B', fontWeight: 700 }}>{z.conf}%</span>
                                        </div>
                                        <div style={{ height: 6, background: '#1E1E1E', borderRadius: 3, marginBottom: 16, overflow: 'hidden' }}>
                                            <div style={{ height: '100%', width: `${z.conf}%`, background: rc.solid }}></div>
                                        </div>

                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#aaa' }}>
                                                <Clock size={14} color="#555" /> <span className="mono" style={{ fontSize: 11 }}>{z.time}</span>
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#aaa' }}>
                                                <Activity size={14} color="#555" /> <span>{z.type}</span>
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#aaa' }}>
                                                <ArrowUpRight size={14} color="#555" /> <span className="mono" style={{ fontSize: 11 }}>{z.vel}</span>
                                            </div>
                                        </div>

                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button className="drawer-btn btn-outline" style={{ flex: 1, padding: '8px 0', fontSize: 12 }} onClick={() => navigate(`/zone/${z.name}`)}>
                                                View Detail
                                            </button>
                                            <button className="generate-btn" style={{ width: 40, padding: 0 }} title="Generate Report" onClick={() => setGenerateZone(z.name)}>
                                                <Download size={14} />
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ background: '#111111' }}>
                                        {['Zone', 'Province', 'District', 'Risk Level', 'Confidence', 'Event Type', 'Last Detected', 'Velocity', 'Actions'].map(col => (
                                ...
                                <tbody>
                                    {filteredZones.map((z, i) => (
                                        <tr
                                            ...
                                            onClick={(e) => {
                                                // Prevent navigation if clicking the report button
                                                if (e.target.closest('button')) return;
                                                navigate(`/zone/${z.name}`);
                                            }}
                                        >
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontWeight: 700, color: '#F5F5F5' }}>{z.name}</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontSize: 13, color: '#999' }}>{z.province}</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontSize: 13, color: '#999' }}>{z.district}</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A' }}><RiskPill risk={z.risk} /></td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontFamily: 'Space Mono', fontSize: 12, color: '#F59E0B', fontWeight: 700 }}>{z.conf}%</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontSize: 13, color: '#F5F5F5' }}>{z.type}</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontFamily: 'Space Mono', fontSize: 11, color: '#666' }}>{z.time}</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A', fontFamily: 'Space Mono', fontSize: 11, color: '#666' }}>{z.vel}</td>
                                            <td style={{ padding: '14px 20px', borderBottom: '1px solid #1A1A1A' }}>
                                                <div style={{ display: 'flex', gap: 8 }} onClick={e => e.stopPropagation()}>
                                                    <button className="drawer-btn btn-outline" style={{ padding: '6px 12px', fontSize: 11, width: 'auto' }} onClick={() => navigate(`/zone/${z.name}`)}>
                                                        View Detail
                                                    </button>
                                                    <button className="generate-btn" style={{ padding: '6px', width: 'auto', background: '#161616', border: '1px solid #2A2A2A', color: '#F5F5F5' }} onClick={() => setGenerateZone(z.name)}>
                                                        <Download size={13} />
                                                    </button>
                                                </div>
                                            </td>
            ...
            {/* Modal */}
            {generateZone && <GenerateModal zoneName={generateZone} onClose={() => setGenerateZone(null)} onGenerated={showToast} />}
